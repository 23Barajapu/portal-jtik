...
class RoutingController extends Controller
{
    ...
    public function d3ak()
    {
        // Ambil data mata kuliah untuk prodi D3AK
        $prodi = Prodi::where('jenjang', 'D3')->first();
        $mataKuliahs = $prodi ? $prodi->mataKuliahs : collect();
        $dosens = \App\Models\Dosen::all();
        return view('prodi.d3ak', compact('mataKuliahs', 'dosens'));
    }
    /**
    ...
    public function d4bd()
    {
        // Ambil data mata kuliah untuk prodi D4BD
        $prodi = Prodi::where('jenjang', 'D4')->first();
        $mataKuliahs = $prodi ? $prodi->mataKuliahs : collect();
        $dosens = \App\Models\Dosen::all();
        return view('prodi.d4bd', compact('mataKuliahs', 'dosens'));
    }
    /**
     * Handle route untuk detail berita
     */
    public function beritaDetail($id)
    {
        $berita = Berita::with('kategori')->findOrFail($id);
        $beritaTerbaru = Berita::with('kategori')->where('id', '!=', $id)->latest()->take(3)->get();
