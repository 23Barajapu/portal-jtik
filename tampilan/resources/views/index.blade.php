...
@section('content')
  <!-- ================= Banner section start ================= -->
    ...
    <div class="row">
        ...
        <div class="vl-herobtn vl-upper">
          <a href={{ route('prodi.index') }} class="theme-btn" style="padding: 12px 32px; font-size:1.1rem; border-radius:32px; font-weight:600; position: relative; z-index: 10;">Lihat Program Studi<span><i class="fa-regular fa-arrow-right"></i></span></a>
        </div>
        </div>
      </div>
      </div>
    ...
    <section class="vl-team vl-team-bg p-relative pt-100 pb-100">
        ...
        <div class="contain">
          <div id="test" class="owl-carousel owl-theme">
            ...
            @endphp
            @foreach($dosen_urut as $dosen)
              <div class="vl-team-sin">
                <div class="vl-team-thumb">
                  <div class="vl-team-shape">
                    <img src="/img/icons/vl-tean-shap1.svg" alt="" />
                  </div>
                  <img class="w-100" src="@if($dosen->foto) @if(str_starts_with($dosen->foto, 'data:image')) {{ $dosen->foto }} @else {{ asset('storage/' . $dosen->foto) }} @endif @else /img/team/vl-team-4.1.png @endif" alt="{{ $dosen->nama }}" />
                ...
                <div class="vl-team-content">
                  <h4 class="title"><a href="#">{{ $dosen->nama }}</a></h4>
                  <span>{{ $dosen->jabatan ?? 'Dosen' }}</span>
                </div>
              </div>
            @endforeach
            {{-- @foreach($tendiks as $tendik)
              <div class="vl-team-sin">
                <div class="vl-team-thumb">
                  ...
                  <img class="w-100" src="{{ $tendik->foto ? asset('storage/' . $tendik->foto) : '/img/team/vl-team-4.1.png' }}" alt="{{ $tendik->nama }}" />
                  <div class="vl-tem-social">
                    <ul>
                      <li><a href="#"><span><i class="fa-brands fa-facebook-f"></i></span></a></li>
                      <li><a href="#"><span><i class="fa-brands fa-linkedin-in"></i></span></a></li>
                ...
                <div class="vl-team-content">
                  <h4 class="title"><a href="#">{{ $tendik->nama }}</a></h4>
                  <span>{{ $tendik->jabatan ?? 'Tendik' }}</span>
                </div>
              </div>
            @endforeach --}}
          </div>
  ...
  <section id="berita" class="vl-service-area vl-gray-bg-4 pt-100 pb-70">
    ...
    <div class="row">
      ...
      <div class="col-lg-4 col-md-6 mb-30 aos-init aos-animate" data-aos="fade-up" data-aos-duration="{{ 900 + ($index * 100) }}">
      <div class="vl-service-icon-box-4 p-relative z-index-1 fix br-8">
        <div class="vl-service-icon-box-4-icon4">
          @if($berita->gambar)
            @if(str_starts_with($berita->gambar, 'data:image'))
              ...
              <span><img src="{{ $berita->gambar }}" alt="{{ $berita->judul }}" style="width: 60px; height: 60px; object-fit: cover; border-radius: 8px;" /></span>
            @else
              {{-- Fallback untuk data lama --}}
              <span><img src="/storage/berita/{{ $berita->gambar }}" alt="{{ $berita->judul }}" style="width: 60px; height: 60px; object-fit: cover; border-radius: 8px;" /></span>
            @endif
          @else
        ...
        <div class="d-flex justify-content-between align-items-center">
          <small class="text-muted">{{ $berita->tanggal->format('d/m/Y') }}</small>
          <a href="{{ route('berita.detail', $berita->id) }}" class="readmore">Baca Selengkapnya <span><i class="fa-regular fa-arrow-right"></i></span></a>
        </div>
        </div>
      </div>
      </div>
