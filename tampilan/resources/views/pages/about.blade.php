                ...
                @foreach ($dosens as $dosen)
                    <div class="col-md-3 col-6" data-aos="zoom-in" data-aos-delay="{{ 200 + $loop->index * 100 }}">
                        <div class="card border-0 shadow-sm text-center p-3 h-100" style="border-radius:16px;">
                            <img src="@if($dosen->foto) @if(str_starts_with($dosen->foto, 'data:image')) {{ $dosen->foto }} @else {{ asset('storage/foto_dosen/' . $dosen->foto) }} @endif @else /img/placeholder-dosen1.png @endif" alt="{{ $dosen->nama }}"
